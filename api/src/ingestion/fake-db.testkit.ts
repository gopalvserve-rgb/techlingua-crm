...
export function makeFakeDb(init: Partial<FakeState> = {}) {
  ...
  const exec = async (sql: string, params: unknown[] = []): Promise<any[]> => {
    ...
    if (s.startsWith('SELECT id, name, code,') && s.includes("NULLIF(meta->>'branch_id'")) {
      // scope-aware course master (loadCourseMaster) — unscoped in tests, so every course is eligible.
      // Client UAT (Aug 2026): a course CODE now resolves too, so the fake rows carry a code.
      return [{ id: 21, name: 'IELTS', code: 'IELTS01', branch_id: null, vertical_id: null },
              { id: 22, name: 'Spoken English', code: 'SPK01', branch_id: null, vertical_id: null }];
    }
